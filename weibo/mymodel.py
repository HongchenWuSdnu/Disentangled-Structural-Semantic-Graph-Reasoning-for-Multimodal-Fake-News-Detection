...
class Multi_Model(nn.Module):
    def __init__(self, bert_path, window_size, num_classes=2,
        ...
        # ===== Paper-aligned: Intra-modal GAT encoders (3 layers each) =====
        ...
        # ===== Paper-aligned: MFM (2-layer GAT on multimodal full graph) =====
        ...
        self.win_attn1 = TextWindowAttnMP(dim=256, dropout=0.1)  # 在 256 维上做一次
        self.win_attn2 = TextWindowAttnMP(dim=128, dropout=0.1)  # 投影到 128 后再做一次
    ...
    def forward(self, input_ids, attention_mask=None, images=None, return_feature=False):
        ...
        if images is not None:
            # images: [B,3,224,224] float, 0~1 (你 dataset 已经是 0~1)
            ...
            # Swin 的 transforms 对 batch tensor 有时不支持逐批处理，所以我们手动 normalize
